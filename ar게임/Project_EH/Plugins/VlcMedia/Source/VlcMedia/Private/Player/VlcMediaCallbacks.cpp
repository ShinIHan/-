...
void FVlcMediaCallbacks::Initialize(FLibvlcMediaPlayer& InPlayer)
{
	...
	//FVlc::AudioSetDelay(Player, 2000);
}
...
void FVlcMediaCallbacks::StaticAudioPlayCallback(void* Opaque, void* Samples, uint32 Count, int64 Timestamp)
{
	...
	int64 dTime = FVlc::Clock() - pTime; //useless code, but tbd well later

	int64 defaultAudioDelay = 750000; //at this time, just set additional audio delay to 750ms. this value calculated exactly to time amout of audio&video stream prepairing

	int64 delay_return = FVlc::Delay(Timestamp + defaultAudioDelay);
	const FTimespan Delay = FTimespan::FromMicroseconds(delay_return);
...
unsigned FVlcMediaCallbacks::StaticVideoSetupCallback(void** Opaque, char* Chroma, unsigned* Width, unsigned* Height, unsigned* Pitches, unsigned* Lines)
{
	...
	//if (FVlc::MediaPlayerGetFps(Callbacks->Player) > 0)

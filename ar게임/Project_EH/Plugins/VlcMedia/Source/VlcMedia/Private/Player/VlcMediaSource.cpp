...
void FVlcMediaSource::AddOption(const ANSICHAR* option) const
{
	if (Media == nullptr)
	{
		UE_LOG(LogVlcMedia, Warning, TEXT("Failed to open media from archive: %s (%s)"), option, ANSI_TO_TCHAR(FVlc::Errmsg()));
	}
	FVlc::MediaAddOption(Media, option);
}

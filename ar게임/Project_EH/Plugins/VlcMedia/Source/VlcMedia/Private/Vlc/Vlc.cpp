...
bool FVlc::Initialize()
{
	...
	VLC_IMPORT(libvlc_clock, Clock)//lets check this function

	VLC_IMPORT(libvlc_media_event_manager, MediaEventManager)
	VLC_IMPORT(libvlc_media_get_duration, MediaGetDuration)
	VLC_IMPORT(libvlc_media_add_option, MediaAddOption)		//libvlc_media_add_option
	...
	VLC_IMPORT(libvlc_audio_set_delay, AudioSetDelay) 
...
int64 FVlc::Delay(int64 Timestamp)
{
	int64 delay = Timestamp - Clock();
	return delay;
}